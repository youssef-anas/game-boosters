...
from valorant.serializers import RankSerializer
...
rank_names = ['', 'IRON', 'BRONZE', 'SILVER', 'GOLD', 'PLATINUM', 'DIAMOND', 'ASCENDANT', 'IMMORTAL']
...
def get_order_result_by_rank(data,extend_order_id):
  ...
  invoice = f'valo-2-{current_rank}-{current_division}-{marks}-{desired_rank}-{desired_division}-{duo_boosting_value}-{select_booster_value}-{turbo_boost_value}-{streaming_value}-{booster_id}-{price}-{extend_order_id}-{timezone.now()}-{choose_agents_value}'
...
def view_that_asks_for_money(request):
  if request.method == 'POST':
    ...
    try:
      ...
      print('request POST', request.POST)
      print('Data is Valid? ', serializer.is_valid())
