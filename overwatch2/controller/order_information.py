...
def get_order_result_by_rank(data,extend_order_id):
    ...
    if booster_id > 0 :
       get_object_or_404(User,id=booster_id,is_booster=True)
        ...
        booster_id = 0
    #####################################
    invoice = f'OVW2-12-D-{current_rank}-{current_division}-{marks}-{desired_rank}-{desired_division}-{duo_boosting_value}-{select_booster_value}-{turbo_boost_value}-{streaming_value}-{booster_id}-{price}-{extend_order_id}-{server}-{booster_champions_value}-{timezone.now()}'
