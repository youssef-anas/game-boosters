...
def get_placement_order_result_by_rank(data):
  ...
  invoice = f'tft-5-P-{last_rank}-{number_of_match}-?-?-?-{duo_boosting_value}-{select_booster_value}-{turbo_boost_value}-{streaming_value}-{booster_id}-{0}-{server}-{price}-{0}-{promo_code_id}-0-0-0-0-{timezone.now()}'
  print('Invoice', invoice)
